...
import 'package:opti_job_app/modules/applicants/ui/widgets/ai_decision_review_dialog.dart';
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ApplicantTile(
                  ...
                  onOpenAiReview: application.id == null
                      ? null
                      : () async {
                          final applicantsRepository = context
                              .read<ApplicantsRepository>();
                          final appliedOverride =
                              await showDialog<bool>(
                                context: context,
                                builder: (_) => AiDecisionReviewDialog(
                                  applicationId: application.id!,
                                  jobOfferId: application.jobOfferId,
                                  repository: applicantsRepository,
                                  initialScore: application.matchScore,
                                ),
                              ) ??
                              false;

                          if (!context.mounted || !appliedOverride) return;

                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Override IA aplicado y auditado correctamente.',
                              ),
                            ),
                          );
                          await context
