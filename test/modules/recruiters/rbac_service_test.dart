...
void main() {
  ...
  group('RbacService', () {
    group('null recruiter', () {
      test('canManageOffers is false', () => expect(rbac.canManageOffers(null), isFalse));
      test('canScore is false', () => expect(rbac.canScore(null), isFalse));
      test('canViewReports is false', () => expect(rbac.canViewReports(null), isFalse));
      test('canManageTeam is false', () => expect(rbac.canManageTeam(null), isFalse));
      test('canInviteMembers is false', () => expect(rbac.canInviteMembers(null), isFalse));
    });

    group('admin role', () {
      final admin = makeRecruiter(role: RecruiterRole.admin);
      test('canManageOffers', () => expect(rbac.canManageOffers(admin), isTrue));
      ...
      test('canInviteMembers', () => expect(rbac.canInviteMembers(admin), isTrue));
    });

    group('recruiter role', () {
      final recruiter = makeRecruiter(role: RecruiterRole.recruiter);
      test('canManageOffers', () => expect(rbac.canManageOffers(recruiter), isTrue));
      test('canScore', () => expect(rbac.canScore(recruiter), isTrue));
      test('canViewReports', () => expect(rbac.canViewReports(recruiter), isTrue));
      test('canManageTeam is false', () => expect(rbac.canManageTeam(recruiter), isFalse));
      test('canInviteMembers is false', () => expect(rbac.canInviteMembers(recruiter), isFalse));
    });

    group('viewer role', () {
      final viewer = makeRecruiter(role: RecruiterRole.viewer);
      test('canManageOffers is false', () => expect(rbac.canManageOffers(viewer), isFalse));
      test('canScore is false', () => expect(rbac.canScore(viewer), isFalse));
      test('canViewReports', () => expect(rbac.canViewReports(viewer), isTrue));
      test('canManageTeam is false', () => expect(rbac.canManageTeam(viewer), isFalse));
      test('canInviteMembers is false', () => expect(rbac.canInviteMembers(viewer), isFalse));
    });
