/**
 * Callable Function: cancelInterview
 *
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        const message: Message = {
            id: messagesRef.id,
            senderUid: uid,
            content: `Interview cancelled by user. Reason: ${reason || 'No reason provided'}`,
            type: 'system',
            createdAt: now,
        };
       ...
       logger.error("Error cancelling interview", error, loggerCtx);
      ...
      throw new functions.https.HttpsError("internal", "Failed to cancel interview");
    }
  }
);
